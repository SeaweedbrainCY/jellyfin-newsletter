...
func getJellyfinNewMovies() []jellyfin.MovieItem {
	return []jellyfin.MovieItem{
		{
			ID:             "fd9416da9026421995b40dae418d2b5d",
			Name:           "Oppenheimer",
		...
		{
			ID:             "7dcf7149f71046d5a50c626e3486259b",
	...
	return []jellyfin.NewlyAddedSeriesItem{
		{
			...
			SeriesID:    "c828b89264f84def88b7dc3d9072a147",
			IsSeriesNew: false,
			NewSeasons: map[string]jellyfin.SeasonItem{
				"eb1224587697415180bf8cb911685398": {
		...
		},
		{
			...
			SeriesID:    "66b82dc8d65544b3b8d75a5eaaa581cd",
			IsSeriesNew: false,
			NewSeasons: map[string]jellyfin.SeasonItem{
				"d4da2014b39a4d2f9f12b45492af4ae6": {
					...
					Episodes: map[string]jellyfin.EpisodeItem{
						"a99874816aa84cd7850bdd962c235685": {
							Name:          "Episode 5",
				...
				"a2b5bd7f7c984f35b27b2c6901982ec1": {
		...
		},
		{
			...
			SeriesID:    "94327c537a324e7c84f45b1a6e71dd35",
			IsSeriesNew: false,
			NewSeasons: map[string]jellyfin.SeasonItem{
		...
		},
		{
			...
			SeriesID:       "3d7b0576370c48d3b7c37c49f612afc9",
			IsSeriesNew:    true,
			NewSeasons:     nil,
			TMDBId:         "1735",
			ProductionYear: 2021,
...
func getExpectedNewMediaTemplateData() newMediaTemplateData {
	title := "New items from " + time.Now().
		AddDate(0, 0, -30).
		Format("Monday 02 01 January 2006 2006-01-02") +
		" to " + time.Now().
		Format("Monday 02 01 January 2006 2006-01-02")
	...
	return &app.ApplicationContext{
		...
		Config: &config.Configuration{
			...
			EmailTemplate: config.EmailTemplateConfig{
				...
				Title:                   "New items from {{.StartDayName}} {{.StartMonthName}} {{.StartYear}} {{.StartDate}} {{.StartDayNumber}} {{.StartMonthNumber}} to {{.DayName}} {{.DayNumber}} {{.MonthNumber}} {{.MonthName}} {{.Year}} {{.Date}}",
				Subtitle:                "Subtitle: New items from {{.StartDayName}} {{.StartMonthName}} {{.StartYear}} {{.StartDate}} {{.StartDayNumber}} {{.StartMonthNumber}} to {{.DayName}} {{.DayNumber}} {{.MonthNumber}} {{.MonthName}} {{.Year}} {{.Date}}",
				JellyfinURL:             "https://jellyfin.example.com",
...
func TestBuildNewMediaTemplateData(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			getAppContextFunc: func() (*app.ApplicationContext, *observer.ObservedLogs) {
				app, obs := getAppContext()
				app.Config.EmailTemplate.SortMode = "name_asc"
				return app, obs
			},
			getExpectedNewMediaTemplateDataFunc: func() newMediaTemplateData {
				expected := getExpectedNewMediaTemplateData()
...
func TestBuildNewMediaEmailHTML(t *testing.T) {
	...
	assert.Contains(t, unescapedHTML, expectedTemplateData.CurrentlyAvailableLabel)
	assert.Contains(t, unescapedHTML, expectedTemplateData.MoviesCount)
	assert.Contains(t, unescapedHTML, expectedTemplateData.MoviesLabel)
	assert.Contains(t, unescapedHTML, expectedTemplateData.SeriesCount)
	assert.Contains(t, unescapedHTML, expectedTemplateData.SeriesLabel)
